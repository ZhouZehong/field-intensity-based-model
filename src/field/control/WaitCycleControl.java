...
/**
 * 用于直接等待机制下每个周期将要进行的操作
 * Created by Hong on 2017/8/10.
 */
public class WaitCycleControl implements Control{
    ...
    public WaitCycleControl(String prefix){
                            ...
                            if (neiborMessage == null){
                                for (Query query : neighborIp.queryCache.queries){
                                    if ((query.getMessageSource() == neighborNode.getID())
                                        ...
                                        maxValueMessage.setHop(maxValueMessage.getHop() + 1);
