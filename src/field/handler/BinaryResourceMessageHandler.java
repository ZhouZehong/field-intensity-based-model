...
/**
 * 固定备份机制（二分法）下处理ResourceMessage并对其进行正确地转发的Handler
 * Created by Hong on 2017/8/17.
 */
public class BinaryResourceMessageHandler extends Handler {
    ...
    public void handleMessage(Node node, int protocolID, Object message) {
        ...
        if (currentField != null){
            if (resourceMessage.getRelayCapacity() < currentField.getFieldStrength())
                ...
                resourceMessage.setRelayCapacity
                        (resourceMessage.getRelayCapacity() - currentField.getFieldStrength());
        }
        else {
            resourceMessage.setRelayCapacity
                    (resourceMessage.getRelayCapacity() - FieldConstructor.decayUpperLimit);
        }
            ...
            if (currentMyLinkable.degree() > 0){
                ...
                for (int i = 0; i < currentMyLinkable.degree(); i++) {
                    ...
                    if ((neighborCacheResourceMessage == null)
                        ...
                        resourceMessage.setRelayCapacity
                                (resourceMessageForCache.getRelayCapacity() / 2);
